...
- [ ] T004 Enforce TypeScript strict mode and verify no implicit any warnings
- [ ] T005 Define/update `src/config.ts` as the only user customization entry point
- [ ] T006 Create/update shared `types.ts` contracts for component props and config

### Constitution Gates for Setup (mandatory)

- [ ] TG01 Document architecture flow `config` -> `types` -> `components` in feature docs
- [ ] TG02 Confirm runtime dependencies stay within approved stack (+ max 1 icon library)
...
- [ ] T007 Establish reusable component foundations without embedding business logic in UI
- [ ] T008 [P] Add accessibility baselines (link `aria-label`, image `alt`, contrast checks)
- [ ] T009 [P] Add mobile-first layout constraints and 320px viewport validation
- [ ] T010 Define SEO baseline (metadata and structured data requirements)
- [ ] T011 Add static deploy validation for `vite build` and GitHub Pages target
...
- [ ] TXXX Revalidate constitution compliance checklist before merge
- [ ] TXXX Run accessibility regression pass (labels, alt text, WCAG AA)
- [ ] TXXX Validate final `vite build` artifact on static hosting target (GitHub Pages)
